...
def get_inspected_data_full_conversion(select_out_of_range=False, convert_datum=True, above_normal_as_surge=False):
    ...
    #merge on lf_ISO_TIME
    df_existing['lf_ISO_TIME'] = pd.to_datetime(df_existing['lf_ISO_TIME'])
    tidal_ranges_df['lf_ISO_TIME'] = pd.to_datetime(tidal_ranges_df['lf_ISO_TIME'])
    df_existing = df_existing.merge(tidal_ranges_df[['lf_ISO_TIME', 'Tidal_Range_peak']], left_on='lf_ISO_TIME', right_on='lf_ISO_TIME', how='left')
    if convert_datum:
        converted_df = convert_datums()
        converted_df['lf_ISO_TIME'] = pd.to_datetime(converted_df['lf_ISO_TIME'])
        df_existing = df_existing.merge(converted_df[['lf_ISO_TIME', 'Converted_value', 'Converted_uncertainty']], on='lf_ISO_TIME', how='left')
    ...
    epoch_correction_df['lf_ISO_TIME'] = pd.to_datetime(epoch_correction_df['lf_ISO_TIME'])
    df_existing = df_existing.merge(epoch_correction_df[['lf_ISO_TIME', 'Offset_to_1983_2001_epoch_m', 'Error_Offset_m']], on='lf_ISO_TIME', how='left')
    ...
    lf_ISO_TIMEs = df_existing['lf_ISO_TIME'].values
    surge_guesses = []
    uncertainties = []
    for idx, row in df_existing.iterrows():
    ...
    return lf_ISO_TIMEs,surge_guesses,uncertainties
...
def get_inspected_data_raw_scenarios():
    ...
    df_first['lf_ISO_TIME'] = pd.to_datetime(df_first['lf_ISO_TIME'])
    df_second['lf_ISO_TIME'] = pd.to_datetime(df_second['lf_ISO_TIME'])
    merged_df = pd.merge(df_first, df_second, on='lf_ISO_TIME', suffixes=('_storm_tide_as_surge', '_subtract_peak_tide'))
    return merged_df
...
def generate_surge_data():
    ...
    if os.path.exists(path):
        ...
        lf_ISO_TIMEs_first, surge_guesses_first, uncertainties_first = get_inspected_data_full_conversion(select_out_of_range=False, convert_datum=True, above_normal_as_surge=True)
        df_surge_first = pd.DataFrame({
            'lf_ISO_TIME': lf_ISO_TIMEs_first,
            'Surge_m': surge_guesses_first,
            'Uncertainty_m': uncertainties_first
        })
        #second scenario treat above normal as unknown datum 
        lf_ISO_TIMEs_second, surge_guesses_second, uncertainties_second = get_inspected_data_full_conversion(select_out_of_range=False, convert_datum=True, above_normal_as_surge=False)
        df_surge_second = pd.DataFrame({
            'lf_ISO_TIME': lf_ISO_TIMEs_second,
            'Surge_m': surge_guesses_second,
            'Uncertainty_m': uncertainties_second
        })
        df_surge_first['lf_ISO_TIME'] = pd.to_datetime(df_surge_first['lf_ISO_TIME'])
        df_surge_second['lf_ISO_TIME'] = pd.to_datetime(df_surge_second['lf_ISO_TIME'])
        merged = df_surge_first.merge(df_surge_second, on='lf_ISO_TIME', suffixes=('_full_conversion_AN_as_surge', '_full_conversion_AN_as_unknown'))
        ...
        raw_scenarios['lf_ISO_TIME'] = pd.to_datetime(raw_scenarios['lf_ISO_TIME'])
        merged = merged.merge(raw_scenarios, on='lf_ISO_TIME', how='left')
